    ...
    | Number(kind, _) ->
        ...
        | UInt64 ->
            Helper.LibCall(
                ...
                "long",
                "toNumber",
                Number(Float64, NumberInfo.Empty),
                args
            )
            |> makeDecimalFromExpr com r targetType
...
let toInt com (ctx: Context) r targetType (args: Expr list) =
    ...
    let emitCast typeTo arg =
        ...
        | Int8 ->
            emitExpr
                None
                (Number(Int8, NumberInfo.Empty))
                [ arg ]
                "(int($0) + 0x80 & 0xFF) - 0x80"
        | Int16 ->
            emitExpr
                None
                (Number(Int16, NumberInfo.Empty))
                [ arg ]
        ...
        | UInt8 ->
            emitExpr
                None
                (Number(UInt8, NumberInfo.Empty))
                [ arg ]
                "int($0+0x100 if $0 < 0 else $0) & 0xFF"
        | UInt16 ->
            emitExpr
                None
                (Number(UInt16, NumberInfo.Empty))
                [ arg ]
                "int($0+0x10000 if $0 < 0 else $0) & 0xFFFF"
        | UInt32 ->
            emitExpr
                None
                (Number(UInt32, NumberInfo.Empty))
                [ arg ]
    ...
    let args =
        ...
        | [ ar; item; start ] ->
            [
                ...
                item
                start
                makeNone (Number(Int32, NumberInfo.Empty))
            ]
        | [ ar; item ] ->
            [
                ar
...
let arrayModule
    ...
    | "IsEmpty", [ ar ] ->
        eq
            (getFieldWith r (Number(Int32, NumberInfo.Empty)) ar "length")
            (makeIntConst 0)
        |> Some
...
let regex
    ...
    | "get_Item" when
        i.DeclaringEntityFullName = "System.Text.RegularExpressions.GroupCollection"
        ->
        Helper.LibCall(
            com,
            "RegExp",
